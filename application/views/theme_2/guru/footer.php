    ...
    $('#myDatepicker2').datetimepicker({
		...
		$("#date_attendance").datetimepicker({format:'YYYY-MM-DD'});
		...
		function countQuestion() { 
			$.ajax({
				type: "GET",
				url: "<?=base_url()?>ApiTeacher/countQuest",
				dataType: "JSON",
				success: function (response) {
					if (response.content>0) {
						$("#count-question").text(response.content);
						$("#count-question").addClass('badge bg-green');
					}
				}
			});
		 }
